...
import com.kyanite.deeperdarker.util.helper.StaticVectorHelper;
...
public class DDBoatRenderer extends EntityRenderer {
    ...
    public void render(Entity entity, float f, float g, PoseStack poseStack, MultiBufferSource multiBufferSource, int i) {
        ...
        poseStack.mulPose(StaticVectorHelper.rotationDegrees(StaticVectorHelper.YP,180.0f - f));
        ...
        if (h > 0.0f) {
            poseStack.mulPose(StaticVectorHelper.rotationDegrees(StaticVectorHelper.XP,Mth.sin(h) * h * j / 10.0f * (float)boatEntity.getHurtDir()));
        }
        if (!Mth.equal(boatEntity.getBubbleAngle(g), 0.0f)) {
            poseStack.mulPose(new Quaternionf(boatEntity.getBubbleAngle(g) * ((float)Math.PI / 180), 1.0f, 0.0f, 1.0f));
        }
        poseStack.scale(-1.0f, -1.0f, 1.0f);
        poseStack.mulPose(StaticVectorHelper.rotationDegrees(StaticVectorHelper.YP,90.0f));
