...
{
    public InfestedSculkBlock(
            Block hostBlock,
            Properties properties
    )
    {
        super(properties.destroyTime(hostBlock.defaultDestroyTime() / 2.0f)
    ...
    )
    {
        if (pEntity instanceof Player player)
        {
            ...
            player.knockback(
                    1.5,
                    player.getX() - pPos.getX(),
                    player.getZ() - pPos.getZ()
            );
            if (pLevel instanceof ServerLevel serverLevel)
                DDEntities.SHRIEK_WORM.spawn(
                        serverLevel,
                        (ItemStack) null,
                        null,
                        pPos,
        ...
        if (pLevel.isClientSide() && pEntity instanceof LivingEntity)
        {
            ...
            for (int i = 0; i < 20; i++)
            {
                ...
                pLevel.addParticle(
                        ParticleTypes.POOF,
                        pPos.getX() + random.nextDouble(),
                        pPos.above()
                            .getY(),
                        pPos.getZ() + random.nextDouble(),
                        sX,
                        sY,
                        sZ
                );
